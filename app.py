...
def main():
    ...
    if (
        ...
        and not os.environ.get("HUGGINGFACE_API_KEY")
    ):
        st.warning(
            "⚠️ No API keys found! Please go to **Model Settings** in the sidebar to configure them before generating."
        )
    ...
    with col1:
        specialty_key = st.selectbox("Medical Specialty", options=list(
            specialty_options.keys()), format_func=lambda x: specialty_options[x])
        val_keywords = st.text_input("Keywords (optional)", help="e.g. 'pregnancy nutrition, safe exercises'")
    with col2:
        format_key = st.selectbox("Output Format", options=list(
            format_options.keys()), format_func=lambda x: format_options[x])
    ...
    selected_client_key = st.selectbox("Client Profile", options=list(
        client_options.keys()), format_func=lambda x: client_options[x])
