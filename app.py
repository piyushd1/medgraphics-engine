    ...
    if st.session_state.topics:
        ...
        if st.button("🚀 Generate Graphics for Selected Topics", type="primary", use_container_width=True):
            ...
            for i, topic in enumerate(selected_topics):
                ...
                # Construct output path safely and ensure it stays within output directory
                output_path = (output_dir / f"render_{safe_title}.png").resolve()
                if not output_path.is_relative_to(output_dir):
                    st.error(f"Security error: Invalid topic title caused path traversal.")
                    continue
