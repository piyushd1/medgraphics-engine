...
def _load_model_config(config_path: str) -> dict:
    """
    Cache the model configuration YAML file to avoid repeated disk reads.
    Using @lru_cache since config_path is a string and the dict is only read.
    """
...
class LLMRouter:
    def __init__(self, config_path="config/models.yaml"):
        ...
        # Performance Optimization: Read from in-memory cache instead of reading disk on every initialization
        self.config = _load_model_config(config_path)
